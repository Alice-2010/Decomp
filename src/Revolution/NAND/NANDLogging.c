...
#include "PowerPC_EABI_Support/MSL/MSL_C/stdlib.h" // IWYU pragma: export
...
inline void callbackRoutine(BOOL success) {
    if (s_callback != NULL) {
        s_callback(success);
    }
}

inline void prepareLine(char line[256], int n, const char* msg) {
    char titleID[64];
    int end = 0;
