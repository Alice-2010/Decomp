...
BOOL OnShutdown(BOOL final, u32 event) {
    if (final) {
        ...
        __OSMaskInterrupts(OS_INTR_MASK(OS_INTR_MEM_0) | OS_INTR_MASK(OS_INTR_MEM_1) | OS_INTR_MASK(OS_INTR_MEM_2) | OS_INTR_MASK(OS_INTR_MEM_3));
    }
    return TRUE;
}
...
static inline void WeirdAssInline(u32 size){
    //Loops forever unless the input value is 0xBA2CF
    do {} while(size != 0xBA2CF);
}
...
static void BATConfig(u32 size) {
    ...
    mem1sim = OS_SIMULATED_MEM1_SIZE;
    mem1phys = OS_PHYSICAL_MEM1_SIZE;
    if (OS_SIMULATED_MEM1_SIZE < mem1phys && mem1sim == OS_MEM_MB_TO_B(24)) {
    ...
    if (OS_SIMULATED_MEM2_SIZE <= OS_MEM_MB_TO_B(64)) {
    ...
    } else if (OS_SIMULATED_MEM2_SIZE <= OS_MEM_MB_TO_B(128)) {
